...
You build the Linux client utilities that sit on top of libreg: `reg`, `sc`,
...
live registry on Linux (everything operates on offline hive files).
...
```
clients/
  Cargo.toml          workspace (cli-core, reg, sc, regedit)
  ...
  sc/                 sc.exe offline service-config CLI
...
1. **Direct-link libreg.** `cli-core` depends on `libreg` by path and drives
   `libreg::logical::Hive` in-process. `reg` and `sc` are self-contained
...
2. **No external crates.** The build environment has no crate registry cache,
   ...
   only on `libreg`; `reg`/`sc`/`regedit` depend only on `cli-core` (plus the
...
4. **Syntax fidelity is the point for `reg` and `sc`.** Match the Windows flag
   grammar (including `sc`'s `key= value` space-after-equals form). Where a
   verb cannot work offline (a running-service verb like `sc start`), fail with
   a clear "not supported on offline hives" message rather than pretend.
